...
- [x] **T8 통계·보고 + 검정력 산정** — `lib/metrics.mjs`(recall@K doc · MRR · nDCG@10 · hit@k · sign test 정확 이항 · bootstrap 10,000 고정 seed · cluster 지정 · 비열등성 CI · Holm) · `report.mjs`(primary 5 endpoint gatekeeping · corpus별 + stratified macro · n/usable) → `out/report.md` · `power.mjs`(pilot 짝지은 Δ SD·불일치율 → 검정력 0.8 · corpus 별 holdout N · 판정 예산 대비) → `suite/POWER.md` + FREEZE.
  ...
  ↳ 코드 완료 2026-08-18(세션39, 리뷰 clean · pool.mjs 3-tier · pool-pass2.mjs(사전 선언 depth-30 표본 = advisor pool-budget REVISE 반영, D2 포화 규칙 대체) · judge-batches.mjs split/merge/status · judge-merge --pass1-only · JUDGING.md 두 단계 · pool 예산 실측 3.6배 → 고정 depth-10 전량 + 표본 depth 30 · 31–100 미판정 · deepsample 100). 실행 = hub pass1 A/B 채점 진행 중(사용자 결정: hub 1차만) → C 조정 → qrels(provisional). ledger 참조.
  ↳ 코드 완료 2026-08-18(세션39, 리뷰 clean · run-upstream.mjs · link-audit-sample.mjs(links/ · second_judge 20% 태그) · link-audit-merge.mjs(reliability κ · exit 15) · provenanceOf = 제품 buildEntityMatcher 와 byte-identical). 실행 = qrels 뒤 run-upstream ×3 · link audit 판정 A/B → merge. 보류 = lib/freeze.mjs 부재 대상 ENOENT(최종 fix wave).
  ↳ 코드 완료 2026-08-18(세션39, 리뷰 대기 · metrics.mjs 전 함수 · report.mjs(두 정답원 authored/judged · judged_depth 게이트 · recall@100 탐색용) · power.mjs → suite/POWER.md · exit 16/17). 첫 실행(작성자 정답 · provisional): K 안전성 FAIL 3/3 · SLO PASS 3/3 · 후보 Δrecall@30 +0.014 · 의미성 PASS 3/3 · 재랭킹 +0.021 · POWER 후보 N 80/134/61 vs 상한 116/114/111. FREEZE 는 qrels 뒤.
  ↳ 완료 2026-08-20(세션40): hub pass1 전량 채점(A/B 104/104 · 진단 6/6 · merge 6/6 · 4,135쌍) → `judge-merge.mjs --pass1-only` = **weighted κ 0.619(A 0.659 · M 0.571) < 게이트 0.67 → exit 8**. 동결 규정(JUDGING.md)의 처방은 루브릭 개정 + 전량 재판정이나 **사용자 결정 = 멈추고 authored 축**(처방 비용 vs 결정 가치 대조) → **qrels 불성립을 `suite/judging-record.json` 에 기록**하고 진행. 불일치 프로파일 = 1,219건 중 0↔1 67% · 0↔2 11% · B 관대 82%(경계 캘리브레이션 차이). uap/hal 및 pass2 채점 = 사용자 결정으로 **취소 유지**. ⚠ 완료 기준의 `κ ≥ 0.67` 은 **미충족이며 그 사실이 기록된 상태**로 닫는다 — 이 change 의 결론 등급이 `provisional` 인 이유가 여기다.
  ↳ 완료 2026-08-20(세션40): `run-upstream.mjs` qrels-부재 모드 수정(커밋 `051737c` · skipped 명시 = 미측정 ≠ 0) 후 3 corpus 실행 = **seed recall 57/56/56 of 60**(93~95%) vs **edge_validity 12/62 · 11/62 · 5/65**(0.194/0.177/0.077) — 세션38 수치 정확 재현 = 교차검증. link audit = 표본 1,739쌍 + 2차 347 · 배치 54 · A 45/45 · B 9/9(재시도 0) → `link-audit-merge` = **가중 정밀도 hub 0.237 CI[0.185,0.289] · uap 0.465 · hal 0.475**(name-provenance ≈1.0 · mention 판정 κ 0.768/0.920/0.818 = **게이트 통과**). provenance pure 테스트 GREEN.
  ↳ 완료 2026-08-20(세션40): 세션39 코드의 리뷰 fix r1(커밋 `9e21a0d` · I1~I5 · mutation 증명) → 범위 한정 재리뷰 **SPEC PASS** (이탈 2건 ACCEPT — pass1+pass2 예산 상한이 추정 ~60 이 아니라 **실측 44/49/47** 이고 후보 3/3 이 depth-30 예산을 초과한다). `out/report.md` · `suite/POWER.md` 재실행 완료. Stage 1 결론 = **provisional 표시** 유지.
...
- [ ] **T10 holdout 전 조건 실행 → pool → 판정 → qrel 동결** — T5·T6 절차 반복(holdout).
  ↳ 🔴 **미실행 — 범위에서 뺀 것이지 못 한 것이 아니다 (사용자 결정, 2026-08-20 세션40).** κ 게이트 미달 시점에 규정 처방(루브릭 개정 + 전량 재판정)의 비용과 결정 가치를 대조한 결과 **결론이 authored 축(upstream)에서 단독으로 나므로** holdout 을 만들지 않고 정지했다. Stage 2 의 holdout suite·실행은 **이 change 범위에서 종결**이며, 재개가 필요하면 새 change 를 연다.
- [x] **T11 결정 → DECISION.md + 후속 change 개설** — `run-decision.mjs`(동결 검사 → gatekeeping 다섯 갈래 → semantics 축 → provisional 이면 remove 금지) → `DECISION.md` · 같은 commit 에 `specs/changes/<selected>/proposal.md`(acceptance: `mode` · 방향/type/confidence · latency/quality · 재평가 명령) · 프레임워크 tracker receipt 경로 기록 · §227 재현 쿼리 2개 관찰.
  ...
  ↳ 완료 2026-08-20(세션40): `run-decision.mjs` 구현 + 테스트 35/35(커밋 `05396a1`) → link-precision ×3 확보 후 `report.mjs` 재실행 → **`run-decision` exit 0 = ① `upstream-first` · grade `provisional` · 3/3 corpus**(edge_validity 0.194/0.177/0.077 < 0.8 이 단독 결정). `DECISION.md` + 후속 change `specs/changes/graph-upstream-build/proposal.md` 를 같은 commit(`2d3c984`)에 두었다. **후속 change 의 1단계 = GraphRAG 계열 철저 조사(착공 전 하드 게이트 · 사용자 요구 원문 포함).** provisional 이라 갈래 (4) remove 는 러너가 거부하도록 걸려 있다.
- [x] **T12 마감** — README 완성 · `out/` 등 commit · 상태 = `evaluation complete`(root repair 는 후속 change).
  ↳ 완료 2026-08-22(세션41): README `## Decision (Task 11)` 절 확인(존재·최신) · 체크박스 정리 · Stage 2 범위 종결 명시 · **상태 = `evaluation complete`**. 🔴 **`evaluation complete` ≠ `root repair complete`** — 랭킹의 근본 수리는 후속 change `graph-upstream-build` 의 몫이고 개시는 사용자 판단이다. 남은 것 = 최종 whole-branch 리뷰(별도) · 브랜치 push/merge(사용자 몫).
