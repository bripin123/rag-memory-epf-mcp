...
| corpus | pilot A+M queries pooled | pass-1 judgements | pass-1 /query | max A+M queries within budget (**pass 1 only**) | + pass-2 (depth-30) judgements | pass-1+2 /query | max within budget (**pass 1 + pass 2**) |
|---|---|---|---|---|---|---|---|
| hub | 60 | 4135 | 68.92 | 116 | 6575 | 178.50 | 44 |
| uap | 60 | 4189 | 69.82 | 114 | 5540 | 162.15 | 49 |
| hal | 60 | 4295 | 71.58 | 111 | 5794 | 168.15 | 47 |

**Which cap applies is a condition, not a detail.** The left-hand cap (hub 116 · uap 114 · hal 111) sizes a holdout judged **pass-1 only**, i.e. to fixed depth 10 — enough for ΔnDCG@10 (rerank) and any hit@k with k ≤ 10. The **candidate** primary endpoint is `recall@30(doc)`, and on **judged** gold that needs `judged_depth: 30` = the predeclared depth-30 **pass 2**, which is spent out of the *same* per-corpus budget (`planPass2` allocates `budget − pass1_rows`). A holdout that has to answer the candidate endpoint on judged gold is therefore capped by the right-hand column (hub 44 · uap 49 · hal 47), not the left one. On **authored** gold the candidate endpoint needs no judging at all and neither cap binds it. The verdict table below is computed against the pass-1-only cap (the frozen rule) and names the depth-30 cap alongside every candidate row.

| corpus | gold | endpoint | N needed | within budget? |
|---|---|---|---|---|
| hub | authored | candidate | 80 | yes (80 ≤ 116) · at depth-30 judging (pass 1 + pass 2, what **judged**-gold recall@30 requires) the cap is 44 → **no (80 > 44)** |
...
| uap | authored | candidate | 134 | **no (134 > 114)** → pre-declare this endpoint `inconclusive` at the frozen budget · at depth-30 judging (pass 1 + pass 2, what **judged**-gold recall@30 requires) the cap is 49 → **no (134 > 49)** |
...
| hal | authored | candidate | 61 | yes (61 ≤ 111) · at depth-30 judging (pass 1 + pass 2, what **judged**-gold recall@30 requires) the cap is 47 → **no (61 > 47)** |
...
**Multiplicity caveat (the Ns above are unadjusted).** Each N is sized at α = 0.05, but `out/report.md` reports these endpoints **Holm-adjusted over a pre-declared family of m=3**. Sizing at the Bonferroni-worst α/3 = 0.0167 raises every one of them: hub/authored/candidate 80→107 · hub/authored/rerank 71→94 · uap/authored/candidate 134→178 · uap/authored/rerank 18→24 · hal/authored/candidate 61→82 · hal/authored/rerank 102→136. That **flips 1** of the verdicts above — hal rerank (136 > cap 111) — from within-budget to over-budget. This line records the exposure; it does not re-decide it. The frozen sizing rule stays the unadjusted one, and changing α is a threshold change (FREEZE.md).
