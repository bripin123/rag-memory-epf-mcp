...
## Classes and units
- K known-item: unit = chunk. Generated by make-known-item.mjs — rowid-uniform stride sample, then a deterministic stride-1 fill pass for any short split; one row per document; split by rowid parity of the document's first chunk; text ≥ 200 chars, query = 180-char excerpt starting at offset min(60, len-220). Documents that are M targets in dev are pinned to dev in K — one K dev row each, first eligible chunk — so that family = document never spans splits; the stride/fill sample of 30 dev + 30 holdout is drawn from the remaining documents.
...
## Lessons applied 2026-08-18 (binding for all M rows, including Stage 2 holdout authoring)
(a) The bridge entity B must not be the target document's own entity (a question naming the target's own file/page is a known-item query in disguise).
(b) The question must not name the target document, its title, or its calendar month/date.
(c) The bridge must be a genuinely shared entity mentioned independently in both documents — not a substring/name-variant coincidence (e.g. `Obsidian` inside `mcp-obsidian`, `Halal-Assistant` inside `AI-Halal-Assistant`).
(d) The target must not be a member-list/CSV/table document (a lookup, not a hop).
(e) Prefer non-role documents (not the project's single current-focus/core-decisions/project-context/index page) as targets when a bridge to a specific document exists.
(f) `expected_entities` must be literal (whole-word) mentions in the cited `source_docs`.
(g) All A/B/C mentions must be independent whole-word mentions, not embedded in longer tokens.
